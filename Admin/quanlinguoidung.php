<?php
  ...
  $tin_nhan_chua_doc = getSQLData('SELECT count(id) count FROM message WHERE read = 0')[0]['count'];
    ...
    $getUserCountQuery = "SELECT count(username) as count FROM _user WHERE upper(hoten) LIKE upper(:partname)";
    $data = array(':partname' => getSearchKey('%') . '%' );
    $userCountSet = getSQLData($getUserCountQuery, $data);
    $totalUserCount = intval($userCountSet[0]['count']);
    ...
    while (($firstPageOfSection + $pageCountOfSection - 2) * $userCountInOnePage >= $totalUserCount)
      $pageCountOfSection--;
    $isLastSection = (($firstPageOfSection + $maxPageCountInOneSection -1) * $userCountInOnePage >= $totalUserCount);
    ...
    $query =  'SELECT username, hoten, email, sotailieu
              FROM _user
              WHERE upper(hoten) LIKE upper(:partname)
              LIMIT :limit_count OFFSET :off_set';
    $data1 = array(':partname' => getSearchKey('%').'%',
                  ':limit_count' => intval($userCountInOnePage),
                  ':off_set'  => ($page - 1) * $userCountInOnePage );
    $usersData = getSQLData($query, $data1);
?>
  ...
  <body class="skin-blue sidebar-mini">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><a href="Admin.php">Dashboard</a> > Quản lí người dùng</li>
          </ol>
        </section>

        <!-- Main content -->
            <section class="col-lg-12 connectedSortable">
                <div class="main-quanli">
                    ...
                    <table class="table table-striped">
                        <thead>
                          <tr>
                            ...
                            <th>Họ và tên</th>
                            <th>Email</th>
                            <th><span style="margin-right: 5px">Số lượng bài đăng </span><!-- <a href=""><i class="fa fa-sort-desc" aria-hidden="true"></i></a><a href=""><i class="fa fa-sort-asc" aria-hidden="true"></i></a> --></th>
                            <th>Xóa</th>
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          foreach ($usersData as $user) {
                            ...
                            echo '<tr>'.
                              '<td><a href="../chitietnguoidung.html?username=' . $user['username'] . '">' . $user['username'] . '</a></td>'.
                              '<td><a href="../chitietnguoidung.html?username=' . $user['username'] . '">' . $user['hoten'] . '</a></td>'.
                              '<td><a href="../chitietnguoidung.html?username=' . $user['username'] . '">' . $user['email'] . '</a></td>'.
                              '<td><a href="../chitietnguoidung.html?username=' . $user['username'] . '">' . $user['sotailieu'] . '</a></td>'.
                              '<td><button type="button" data-toggle="modal" data-target="#delModal"  data-username="' .
                               $user['username'] .'" data-hoten="' .
                               $user['hoten'] .'" id="button_delete"><i class="fa fa-trash-o" aria-hidden="true"></i></button></td>'.
                            '</tr>';
                          }
                          ?>
                        </tbody>
                      ...
                      <script type="text/javascript">
                        $('button[data-target=#delModal]').click(function(arg) {
                         $('#user-fullname').html($(this).attr('data-hoten'));
                         $('#real-delete').attr('data-value', $(this).attr('data-username'));
                        })
                      </script>
                      <div style="text-align: center;">
                        ...
                        <?php
                          ...
                          if($firstPageOfSection > $pageCountOfSection && $pageCountOfSection > 0)
                            echo '<li><a href="quanlinguoidung.php?page=' . ($firstPageOfSection - $pageCountOfSection) . getSearchKey('&q='). '">&laquo;</a></li>';
                           for ($i= $firstPageOfSection; $i < $firstPageOfSection + $pageCountOfSection; $i++) { 
                            echo '<li ' . (($i == $page)?'class="active"':'') .'><a href="quanlinguoidung.php?page=' . $i .getSearchKey('&q=').'">'. $i .'</a></li>';
                           }
                           if(!$isLastSection)
                            echo '<li><a href="quanlinguoidung.php?page=' . ($firstPageOfSection + $pageCountOfSection) .getSearchKey('&q='). '">&raquo;</a></li>';
                          ?>
                      </ul>
                      </div>
                  </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Bạn thực sự muốn xóa người dùng <span id="user-fullname"></span></p>
          </div>
    ...
    <script type="text/javascript">
      $('#real-delete').on('click', function(){
        $.ajax({url: '../delete.php?username='+$(this).attr('data-value'), success: function(result){
            if (result == '1') {
              //success
              location.reload();
            }else{
